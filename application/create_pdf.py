...
def create_pdf(image_path='static/images', save_path='static/pdf', file_name='人教版数学一年级上册预习卡'):
    """
    Description: 根据图片列表生成 pdf 文件，图片需要以数字命名
...
def create_pdf_two(image_path='static/images', save_path='static/pdf', file_name=''):
    """
    Description: 根据图片列表生成多个正反 pdf 文件，图片需要以数字命名，并且根据奇偶分成两个数组，然后以10个为一组
    * @param image_path 图片路径
    * @param save_path pdf保存路径
    * @param file_name pdf文件名
    """
...
def create_pdf_interval_two(image_path='static/images', save_path='static/pdf', file_name=''):
    """
    ...
    * @param file_name pdf文件名
    ...
    # 5.将多个图像转换为PDF
    file_path = save_path + '/' + file_name + '_正面.pdf'  # 正面文件路径
    with open(file_path, "wb") as f:
        f.write(img2pdf.convert(images_1))

    # 6.反面PDF
    file_path = save_path + '/' + file_name + '_反面.pdf'  # 反面面文件路径
    with open(file_path, "wb") as f:
